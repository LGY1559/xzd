  ...
  <div class="head flex">
    <div class="flex items-center gap-3">
      ...
      <div class="gap-4 flex items-center">
        ...
        <span class="text-sm">实时天气：{{ weather }} {{ temperature }}°C {{ winddirection }} {{ windpower }}</span>
      </div>
    </div>
    <div class="flex items-center gap-3 ">
      <i class="iconfont icon-tishi_mian" @click="showPopup"></i>
      <i class="iconfont icon-tianjia" v-show="false"></i>
    </div>
    <div v-if="isPopupVisible" class="popup">
      ...
      <div class="popup-content">
        ...
        <button @click="hidePopup" style="background-color:rgb(0 78 113); width: 100px;margin-top: 1rem;" class="text-white">
          关闭
        </button>
      </div>
    </div>
...
watch(() => weatherStore.cityid, (newCityid) => {
